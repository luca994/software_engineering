...
public class CostruisciEmporioConTesseraTest {
	...
	public void testEseguiAzioneSenzaSufficientiAiutanti() throws FuoriDalLimiteDelPercorso, CartePoliticaIncorrette, NumeroAiutantiIncorretto, EmporioGiaCostruito{
		//Aggiungo una tessera valida a g1
		g1.getTessereValide().add(giocoTester.getTabellone().getRegioni().get(0).getTessereCostruzione().get(0));
		giocoTester.getTabellone().getRegioni().get(0).nuovaTessera(giocoTester.getTabellone().getRegioni().get(0).getTessereCostruzione().get(0));
		//Setto il creatore dell'azione
	...
	@Test
	public void testAggiuntaPuntiQuandoGliEmporiSonoFiniti() throws FuoriDalLimiteDelPercorso, CartePoliticaIncorrette, NumeroAiutantiIncorretto, EmporioGiaCostruito{
		//Aggiungo una tessera valida a g1 e sostituisco il bonus con uno che non modifica i punti vittoria
		g1.getTessereValide().add(giocoTester.getTabellone().getRegioni().get(0).getTessereCostruzione().get(0));
		g1.getTessereValide().get(0).getBonus().clear();
		g1.getTessereValide().get(0).getBonus().add(new BonusMoneta(giocoTester.getTabellone().getPercorsoRicchezza(), 1));
		giocoTester.getTabellone().getRegioni().get(0).nuovaTessera(giocoTester.getTabellone().getRegioni().get(0).getTessereCostruzione().get(0));
		//Setto il creatore dell'azione e modifico il bonus della citta che non modifica i punti vittoria
		List<Citta> temp =new ArrayList<>(g1.getTessereValide().get(0).getCitta());//Creata e convertita solo localmente perchè è più comoda del set
		creaAzioniTester.setCitta(temp.get(0));
