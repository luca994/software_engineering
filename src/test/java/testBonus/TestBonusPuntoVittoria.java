...
public class TestBonusPuntoVittoria {
	...
	public void testAzioneBonusConValoriInRange() {
		...
		assertEquals(posizioneIniziale+incremento,giocoTester.getTabellone().getPercorsoVittoria().posizioneAttualeGiocatore(g1));
	}
	@Test
	public void testAzioneBonusConValoriFuoriRangePositivi() {
		...
		bonusPuntoVittoria=new BonusPuntoVittoria(giocoTester.getTabellone().getPercorsoVittoria(), incremento);
		bonusPuntoVittoria.azioneBonus(g1);
		assertEquals(giocoTester.getTabellone().getPercorsoVittoria().getCaselle().size()-1,giocoTester.getTabellone().getPercorsoVittoria().posizioneAttualeGiocatore(g1));
	}
