...
public class ServerRMIView extends ServerView implements ServerRMIViewInterface {
	...
	public void update(Object cambiamento, Giocatore attributo) {
		 ...
		 * { String parametro = client.impostaBonus((Bonus) cambiamento); try {
		 * this.notificaObservers((Bonus) cambiamento, parametro); } catch
		 * (InterruptedException e) { e.printStackTrace(); } } else if
		 * (this.giocatore.equals(attributo)) { inviaOggetto(cambiamento); }
		 */
	}
